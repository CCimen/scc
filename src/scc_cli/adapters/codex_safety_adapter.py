"""Codex-specific safety adapter — UX formatting and audit emission."""
...
class CodexSafetyAdapter:
    """Wraps SafetyEngine with Codex-specific user messages and audit events."""
    ...
    def check_command(self, command: str, policy: SafetyPolicy) -> SafetyCheckResult:
        ...
        if verdict.allowed:
            user_message = "[Codex] Command allowed"
        else:
            user_message = f"[Codex] Command blocked: {verdict.reason}"
