...
from ...ui.git_interactive import clone_repo
...
def start(
    ...
    if start_plan.sync_error_message:
        panel = create_warning_panel(
            "Marketplace Sync Failed",
            start_plan.sync_error_message,
            "Team plugins may not be available. Use --dry-run to diagnose.",
        )
        print_with_layout(console, panel, constrain=True)
    elif start_plan.sync_result:
        if start_plan.sync_result.warnings:
            console.print()
            for warning in start_plan.sync_result.warnings:
                print_with_layout(console, f"[yellow]{warning}[/yellow]")
            console.print()
        if start_plan.sync_result.plugins_enabled:
            print_with_layout(
                console,
                f"[green]{Indicators.get('PASS')} Enabled "
                f"{len(start_plan.sync_result.plugins_enabled)} team plugin(s)[/green]",
            )
        if start_plan.sync_result.marketplaces_materialized:
            print_with_layout(
                console,
                f"[green]{Indicators.get('PASS')} Materialized "
                f"{len(start_plan.sync_result.marketplaces_materialized)} marketplace(s)[/green]",
            )
                ...
                if workspace_contexts and not user_dismissed_quick_resume:
                    ...
                    while True:
                        ...
                        match result:
                            ...
                            case QuickResumeResult.BACK:
                                # User pressed Esc - go back to workspace picker
                                workspace = None
                                break
                            ...
                            case QuickResumeResult.NEW_SESSION:
                                # User pressed 'n' or selected "New Session" entry
                                console.print()
                                break  # Exit workspace QR, continue to wizard
