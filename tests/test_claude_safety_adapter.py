"""Unit tests for ClaudeSafetyAdapter."""
...
) -> tuple[ClaudeSafetyAdapter, FakeSafetyEngine, FakeAuditEventSink]:
    engine = FakeSafetyEngine()
    if verdict is not None:
        engine.verdict = verdict
    sink = FakeAuditEventSink()
    return ClaudeSafetyAdapter(engine=engine, audit_sink=sink), engine, sink
...
class TestBlockedCommandEmitsWarningAuditEvent:
    def test_blocked_command_emits_warning_audit_event(self) -> None:
        ...
        assert event.subject == "claude"
        assert event.metadata["provider_id"] == "claude"
        assert event.metadata["command"] == "git push --force"
        assert event.metadata["verdict_allowed"] == "false"
        assert event.metadata["matched_rule"] == "force-push"
        assert event.metadata["command_family"] == "destructive-git"
...
class TestAllowedCommandEmitsInfoAuditEvent:
    def test_allowed_command_emits_info_audit_event(self) -> None:
        ...
        adapter.check_command("ls -la", _POLICY)
