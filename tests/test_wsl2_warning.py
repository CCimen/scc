...
def test_wsl2_warning_emitted_in_non_interactive(tmp_path: Path) -> None:
    ...
    from scc_cli.commands.launch import _validate_and_resolve_workspace

    workspace = tmp_path / "repo"
    workspace.mkdir()

    with (
        patch("scc_cli.commands.launch.platform_module.is_wsl2", return_value=True),
        patch(
            "scc_cli.commands.launch.platform_module.check_path_performance",
            return_value=(False, "warning"),
        ),
        patch("scc_cli.commands.launch.is_interactive_allowed", return_value=False),
        patch("scc_cli.commands.launch.Confirm.ask") as mock_confirm,
