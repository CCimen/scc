...
from scc_cli.commands.launch import _warn_if_non_worktree
...
def test_warns_when_in_main_repo() -> None:
    ...
    with (
        patch("scc_cli.commands.launch.git.is_git_repo", return_value=True),
        patch("scc_cli.commands.launch.git.is_worktree", return_value=False),
...
def test_no_warning_for_worktree() -> None:
    ...
    with (
        patch("scc_cli.commands.launch.git.is_git_repo", return_value=True),
        patch("scc_cli.commands.launch.git.is_worktree", return_value=True),
        patch("scc_cli.commands.launch.print_human") as mock_print,
    ):
        _warn_if_non_worktree(workspace, json_mode=False)
...
def test_no_warning_for_non_repo() -> None:
    ...
    with (
        patch("scc_cli.commands.launch.git.is_git_repo", return_value=False),
        patch("scc_cli.commands.launch.print_human") as mock_print,
    ):
        _warn_if_non_worktree(workspace, json_mode=False)
