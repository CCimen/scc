...
| D015 | M003/S04 planning | governance | How enterprise network governance composes across org, team, project, user, and session scope | Org policy owns the baseline network mode, non-overridable deny overlays, the named destination-set catalog, and delegation rules. Team policy may widen only within delegated bounds, typically by selecting approved named sets. Project and user scopes may narrow only. Every workspace/session runs under exactly one active team context; SCC must not implicitly union network access from multiple teams for one session. In enforced modes, topology plus proxy policy remain the hard control while wrappers provide defense-in-depth, UX, and audit. | This keeps SCC easy to explain and easy to maintain for enterprise teams: centrally governed widening, predictable auditability, simple user context switching, and named destination sets instead of ad hoc host lists. It also avoids overselling isolation while preserving a strong security story. | Yes | collaborative |
