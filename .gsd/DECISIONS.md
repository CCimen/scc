...
| D017 | M004/S02 override | safety | How M004/S02–S05 should scope the safety architecture and wrapper enforcement claims | Keep the safety architecture narrow and truthful across M004/S02–S05. S02 wrapper scope is exactly 7 tools (git, curl, wget, ssh, scp, sftp, rsync) — no package managers, cloud CLIs, kubectl, terraform, or broad command families. In enforced web-egress modes, topology + proxy policy remain the hard network control; network-tool wrappers are defense-in-depth, early UX, and audit only — never claim wrappers are the primary enforcement plane. Provider-native integrations are adapter-owned and additive only. One active team context per session/workspace in diagnostics and safety surfaces — no implicit union of team access. M004 maintainability work stays local to touched files; do not pull M005-style broad cleanup forward. | User override during M004/S02 planning to prevent scope creep and ensure all safety claims in docs, diagnostics, and tests match actual enforcement capabilities. The wrapper layer is valuable as defense-in-depth and audit, but the topology + proxy sidecar (D014) is the hard control for network isolation. Keeping the distinction explicit in planning documents prevents inflated security claims and keeps each enforcement layer honest about its role. | Yes | human |
| D018 | post-M004 architecture clarification | architecture | What the canonical policy object should be for Claude/Codex reusable workflows and integrations | Govern one provider-neutral artifact catalog plus bundle model in core. Skills are the shared portability surface, MCP definitions stay provider-neutral where possible, and provider-native hooks/plugins/marketplaces are adapter-owned render targets only. Teams should enable bundles once, then adapters render Claude or Codex native assets from the same plan. | This avoids dual team configs, keeps SCC truthful about portability, prevents the current Claude-shaped plugin model from hardening into a cross-provider core contract, and preserves a clean adapter boundary as Codex support grows. | Yes | collaborative |
| D019 | post-M004 architecture clarification | architecture | Whether Claude and Codex should be forced into one plugin schema | Treat Claude and Codex plugin surfaces as intentionally asymmetric. Claude plugins can carry richer native surfaces; Codex plugins bundle skills/apps/MCP and rely on separate rules/hooks/config/instruction layers. One SCC bundle should project to the closest truthful native shape per provider. | This keeps the native surface model honest, prevents future technical debt from a fake shared plugin abstraction, and preserves the "one team policy, many adapter renderers" direction. | Yes | collaborative |
| D020 | post-M004 architecture clarification | architecture | What source-of-truth and packaging model should back team-shared bundles | Keep one approved SCC bundle source per team pack, referenced by ID in team policy. Render or publish Claude/Codex-native marketplace/plugin artifacts from that source instead of making provider marketplaces the canonical config input. | This gives the team the UX of "one team pack" while preserving governance, pinning, auditability, and cross-provider maintainability. | Yes | collaborative |
